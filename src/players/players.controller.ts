...
export class PlayersController {
    ...
    @Get(':_id')
    async getPlayerById(@Param('_id', ValidationParametersPipe) _id: string): Promise<Player> {
        return await this.playersService.getPlayerById(_id);
    }

    @Put(':_id')
    @UsePipes(ValidationPipe)
    async updatePlayer(@Body() updatePlayerDto: UpdatePlayerDto, @Param('_id', ValidationParametersPipe) _id: string): Promise<void> {
        await this.playersService.updatePlayer(_id, updatePlayerDto);
    }

    @Delete(':_id')
    async deletePlayer(@Param('_id', ValidationParametersPipe) _id: string): Promise<void> {
